...
%(RLS) spanning hours in order to count cell division and rectify when defects are occurred, which can incur high labor costs, tedious, loss and degradation of quality in the desired collected results.Here, we propose a circular plotting method to visualize the dynamic cellular events. (Describe the circular plot method)

%yeast cells dynamic development and simplify the countability of RLS. We applied deep learning techniques to extract features from detected cells and developed a R package to visualize RLS for every single trap of microfluidics device. 
...
%Biology field is promptly changing from a bench-top paradigm to a computational science where, biologists are automating large-scale experiments to capture large collections of results as digital images \cite{ r1, r2}. For example, unitizing an algorithm, the temporal dynamics of cells can then be derived and simplified. 
...
Visualization can offer an efficient path to study the data obtained from live-cell imaging including high dimensional biological data. However, it has not adequately been developed in time-lapse microscopic images with an interactive visualization. In other word, there is a knowledgeable gap between some biology areas and suitable visualization tools, in particular, visualizing the dynamic development of cells and lifespan. Furthermore, many of the experimental results from these microscopic images are based on numerical numbers which require expertise to verify the validation of the outcome. 

Microfluidics device is an ultra-small structure with micro fluid channels offer faster and reliable results in comparison to traditional methods \cite {r2.1,r2.2}. Duo to theirs functionality, microsacle or nonoscale dimension, they can be used in many applications such as drug delivery, cell monitoring, cell division, virus inspection and so on \cite{r2.3,r2.4}. Time-lapse microfluidics images amplifies biologists ability to experimentally image live-cell during the developing progress \cite{r3}. Recently, a new network model of cellular aging was proposed based on the cellular aging process of yeast cells \cite{ref4.2}. For instance, Jo et al. \cite{\r13} investigated  the high-throughput analysis of dividing yeast cells using microfluidics device. 

Cell division enumeration is budding yeast process that is one of essential factor in aging where single mother cell can be accomplished prior to inactivation, also known as replicative lifespan (RLS). Determining the lifespan of cells is time consuming process and in many cases is based on manual operation, such as cells microdissection \cite{ref08}. Accordingly, visualization of cell growth and division potentially can offer great point of view to estimate RLS of cells (e.g, yeast) with would have an effective impact in the biology. 

Hence, structural information in high-resolution is one of the essential factors in understanding the molecular dynamics and cell functionalities from microfluidics-based microscopy images. Yet, it is challenging to visualize and interpret the data gathered through microfluidics-based microscopy interactively.
...
% To this end, we developed the a visualization method to interpret microfluidics images including the cell division process. We developed an R package that offers region of interest visualization for microfluidics images. For instance of a microfluidics device with traps, all time-lapse trap images could be visualized for analysis and cell monitoring especially, budding cell development. 
...
% Yet, existing visualization methods are dispersed among diverse approaches, many methods introduced in the biomedical field. Due to


% Microfluidics devices are grouped into different categories depending on application, dimension and force control. In general, there are two type force control for these devices. Passive type when fluids and particles are subject to innate forces such as traction, inertial and wall-induced forces. Active type when there are extrinsic forces, which apply on the fluids and particles inside the microfluidics device. 
...
All time-lapse microfluidics images are captured with IX-81 Olympus inverted microscope equipped with an Olympus camera (DP72 CCD) operating by cellSens software. The experimental process took around 96 hours at temperature of 86$^{\circ}$F (30$^{\circ}$C). Each channel has 520 traps inside microfluidics chamber and each row has 7 or 6 traps consecutively. Channels are labeled with numbers right-side for capturing alignment. The order of rows could be different depending on capturing adjustment. For instant, if the camera set on guideline 10 and 20, the first row of captured image starts with 7 traps and so on.
The experimental ended with capturing 391 images with 10 minutes interval in between images. 

\subsection{Data Pre-processing}
Since microfluidics images have relatively low resolution and are considered as challenging task for visualization, we decided to partition every single image to sub-image based on individual trap. Traps are labeled from time-point 1 to time-point 391 as illustrated in Fig.\ref{fig:micro}. All sub-images positioned with respect to local traps at 60x60 resolution. Images are captured at four locations of the microfluidics device. 1st position: camera covered guideline 10-20. 2nd position: camera covered guideline 30-40. 3rd position: camera covered guideline 50-60. 4th position: camera covered guideline 70. In this work, we have considered the dataset from 1st position as shown in Fig.\ref{fig:micro}. We used You Only Look Once (YOLOv3) \cite{ref20} machine learning model for image features extraction. We tested 391 sequence sub-images for each trap, extract and label them as following: trap number (trap\_num),time number (time\_num), total number of cell (total\_objs), cell x coordinate (obj\_X), cell y coordinate(obj\_Y) and cell area (obj\_area) as shown in Fig.\ref{fig:table}. The features of each image are collected orderly by row and increases by number of available objects in the image. For instance, the features of first image input with four cells are collected by indicating four rows for each cell features and assigning time number (time\_num) one for all of them. This labeling method continuously followed for the rest of images.



% 100  randomly selected sub-images are used for training dataset. Since the number of cells on each image could have variety number of cells, We choose images with any number of cells inside trap and images with blank trap (no cells). We then train a model for two classes; any image with cell labeled as cell class and any image without cell labeled as trap class. For feature extraction, We considered some models such as Single Short Detector (SSD), Faster R-CNN and and You Only Look Once(YOLOv3) \cite{ref19, ref20}. The indication of the comparison shows that YOLO has more efficient actuary. 

 
\subsection{Distance calculation and time-points conversion}

We did feature extraction for 104 available traps as illustrated in Fig.\ref{fig:micro}. In addition, we calculated cell distance using euclidean equation:
