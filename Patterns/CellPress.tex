...
In this section, the simplicity and practicality of the $\mu$Polar tool in the contour expansion will be discussed. In Fig.\ref{fig:explain} example, cells are shown in five color codes depending on the number of available cells in the image. The black color represents a single mother cell(mC), light blue represents two cells, pink color represents three cells, green color represents four cells and purple color represents five cells. At first glance, it appears that the number cells at most of the time-points is two and many of the divisions are identifiable. For instance, the initial division started with dC below mC" event and changed to "dC above mC" event shortly (#1 and #2). During these time-points, there are small variations in the number of cells (pink and green colors)for a short period of time and followed by two cells. This process continued for some time-points and the number of cells increases from two to three cells (pink color) for a longer period of time until there was not any cell available inside trap (#3). It is noticeable that the distance between cells and the reference point increased meaning cells are more likely located at top of the trap.

After this transition (2 cells to 3 cells), the division started again with "dC below mC" event and continued longer than a similar event at earlier time-pints. #4 represents an example of a completed division cycle point from two cells. These division cycle points have occurred several times in this example which this countability is useful for estimating the number of divisions by visualization. In addition, the overcrowded time-points (#6) can be identified by the corresponding color code. In general, dark colors represent cell location and lighter colors represent the distance between cells and reference points. This method is useful for tracking cells by the following cell with the same color at each time-point. For example, #5 shows a growing dC below mC which can be tracked by dark-red color at each time point. Since the area of the cell can be applied to $\mu$Polar function, these cells growing time-points can be visualized with corresponding cell size at each time-point as well as color codes. Senescence time-points (#7) are demonstrating that two grown cells have close to each other for sometimes without noticeable variation in cell size or division. The #8 represents a single cell inside the trap close to the end of the experiment work and potentially considered as a dead cell. This interpretation of cell development and comparability of time-points can be beneficial for cell monitoring development and cell migration. 
