...
%In fact, there is a demand for suitable visualization tools in the biology field where microscopic images are interpreting the dynamic development of cells and lifespan.

In the study of cellular aging, time-lapsed microfluidic microscopic imaging has provided unprecedented quantitative details on changes of cell characteristics during aging. The budding yeast is an effective model for cellular aging [Cite a review, Qin05, Qin19]. 
%Recently, a new network model of cellular aging was proposed based on the cellular aging process of yeast cells \cite{ref4.2}. 
...
The yeast replicative lifespan is defined as the number of cell divisions that a single mother cells can accomplish before it cease to divide. 
Determining the replicative lifespan of dividing yeast cells is time-consuming and is traditionally  measured through manual micro-dissection \cite{ref08}. The microfluidic approach, as a high-throughput method, generated hundreds of time-lapse microscopic images, which converted the old challenge of manual dissection into a new challenge of time series image data analysis. One way to tackle 
...
\hline
\\
$\mu$Polar(data,dREF,tmPoint,titile,numAtics,AticFmt,pREF,vArea,cTrack,offset)\\
\\
\hline
\hline
\\
...
\hline
\\
(I) & data &       Input dataset for file containing Time, coordinate, Area (optional) and RLS (optional)\\
\\
(II) & dREF &      Given reference point on image (x, y) to calculate distance from cell\\
\\
(III)& tmPoint &   Given starting time-point and ending time-point for a range of plot (default = all time-points)\\
\\
(IV)& title &      Given title (default = no title)\\
\\
(V)& numAtics &    Number of Evenly Spaced Angle Tic Lines\\
\\
(VI)& AticFmt &    Fmt for Angle Tic Text (e.g. $T= \%2.2f $)\\
\\
(VII)& pREF &      Numeric value for plot reference line \\
\\
(VIII)& vArea &    Use cell area ( if available) for cell development (e.g. ON)\\
\\
(IX)& cTrack &     Vector of colors using for color code tracking  (e.g. ON)\\
\\
(X)& offSet &      numerical value for adjust largest distance from plot edge\\
\\
\end{tabular}
\label{table:desc}
\end{table*}
...
\section{Discussion}
In this section, the simplicity and practicality of the $\mu$Polar tool in the contour expansion will be discussed. Since there are some $\mu$Polar examples in \nameref{S1_Fig}, we focused on visualization result of "Trap98" which represented in Fig.\ref{fig:explain}.
